/**
 * `ai_mcp_connections` É SERVER-SIDE ONLY, E ISSO SE MEDE.
 *
 * ## O que se pagaria
 *
 * A linha guarda o cabeçalho de acesso (`auth_header_value_encrypted`, ex.:
 * "Bearer ...") do servidor MCP que a organização conectou (n8n, DeepWiki,
 * sistema de cliente). Quem tem esse valor fala com o servidor externo como
 * se fosse a organização: lê e, dependendo do MCP, escreve nos sistemas dela.
 * A anon key vai para o browser.
 *
 ...
 * PostgREST a tabela que guarda a credencial. Deny-all é MAIS restritivo que
 * isolamento por tenant, não menos.
 *
 * Irmão declarado de `credencial-de-anuncios-e-server-side.test.ts`, que é o
 * molde, e do desenho de `platform_meta_app` (0257) e da UAZAPI (0261).
 *
 * A ligação entre os dois arquivos é MECÂNICA, não um comentário que alguém
 * precise lembrar de ler: o caso "não está na lista do outro" importa a
 * constante `TABLES` dele e reprova se alguém acrescentar `ai_mcp_connections`
 * lá. `tests/invariants/**` é congelado por hook de pre-commit, então a guarda
 * que liga os dois tinha de morar do lado que não é congelado: este.
 *
 * ## Por que RLS-sem-policy não basta sozinha
 *
 * O `supabase/baseline.sql` traz `ALTER DEFAULT PRIVILEGES ... GRANT ALL ON
 * TABLES TO anon` / `... TO authenticated`, e eles valem para toda tabela
 * criada DEPOIS deles, isto é, para todo apêndice novo. Tabela nova nasce
 ...
 * barrou" de "o privilégio não existe") nos quatro comandos, não só na leitura.
 */
...
/** A tabela da migration 0901 (fork Hiperbold, faixa 09xx). */
const TABELAS = ["ai_mcp_connections"] as const;
...
describe("a lista do `rls-isolation` e esta não se sobrepõem", () => {
  it.each(TABELAS)("`%s` NÃO está em `TABLES` do rls-isolation, e não pode entrar", (tabela) => {
    // Acrescentar `ai_mcp_connections` lá NÃO faria o teste medir mais: `countAs`
    // roda `set role authenticated`, e com privilégio NENHUM o psql devolve
    ...
    // SERVIR pelo PostgREST a tabela que guarda a credencial do servidor MCP.
    //
    ...
    expect(
      ...
      `\`${tabela}\` entrou em TABLES do rls-isolation. Ela é deny-all (RLS ligada, ` +
        ...
        "e criar policy para consertá-lo expõe a credencial do MCP pelo PostgREST. A prova " +
        "dela é ESTE arquivo.",
    ).toBe(false);
  });
});
...
describe.each(TABELAS)("o PostgREST não serve `%s`", (tabela) => {
  ...
  it("`authenticated` também não tem, nenhuma tela lê isto pelo client de sessão", () => {
    // Quem lê é o servidor, com o admin client, filtrando organization_id à
    // mão (`lib/ai/mcp-externo/conexoes.ts`).
  ...
  it("`service_role` CONTINUA com privilégio, controle positivo do papel que usa", () => {
    // Se ele sumir, a tela para de gravar a conexão e o produto degrada em
  ...
  it("`anon` é BARRADO ao inserir", () => {
    esperaBarrado(
      "anon",
      `insert into public.${tabela} (organization_id, slug, name, url) ` +
        `values (gen_random_uuid(), 'ok', 'nome', 'https://exemplo.com')`,
    );
  });

  it("`authenticated` é BARRADO ao inserir", () => {
    esperaBarrado(
      "authenticated",
      `insert into public.${tabela} (organization_id, slug, name, url) ` +
        `values (gen_random_uuid(), 'ok', 'nome', 'https://exemplo.com')`,
  ...
  it("`anon` é BARRADO ao atualizar", () => {
    esperaBarrado("anon", `update public.${tabela} set is_active = false`);
  });

  it("`authenticated` é BARRADO ao atualizar", () => {
    esperaBarrado("authenticated", `update public.${tabela} set is_active = false`);
  });

  it("`anon` é BARRADO ao apagar", () => {
    esperaBarrado("anon", `delete from public.${tabela}`);
  });

  it("`authenticated` é BARRADO ao apagar", () => {
    esperaBarrado("authenticated", `delete from public.${tabela}`);
  });
  ...
  it("não há policy nenhuma, servir esta tabela nunca foi a intenção", () => {
    ...
    expect(
      quantas,
      "alguém criou policy: a tabela passa a ser SERVIDA pelo PostgREST, e a credencial " +
  ...
  it("é tenant-aware de verdade, `organization_id` NOT NULL com FK em cascata", () => {
    // A tabela guarda a conexão MCP de UMA organização. Sem a coluna (ou com
    // ela nullable) o filtro manual do handler não teria em que se apoiar, e
    // apagar a organização deixaria a credencial órfã viva no banco.
    const coluna = sql(`
