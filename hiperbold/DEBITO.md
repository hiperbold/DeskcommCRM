...
- [ ] **D-023 · Conferir de ponta a ponta a saída que outro sistema manda pela instância.** Implementado em 16/09/2026: o webhook deixou de ser registrado com `excludeMessages: ["wasSentByApi"]` (webhook antigo com o filtro é atualizado pelo id ao reconectar), a saída mandada pela API entra na conversa com `metadata.via_api`, pausa a IA, e o eco do próprio envio é apagado pelo envio por id exato (`removerEcoDoProprioEnvio`), sem pausar a IA quando há envio nosso em voo com o mesmo texto. Provado por teste (inclusive com mutação), NÃO na instância real: o servidor da UAZAPI estava fora do ar (HTTPS sem resposta, verificador externo confirmando). Falta: reconectar a S9 no CRM, mandar mensagem pelo n8n/API e pelo CRM e conferir que a primeira aparece uma vez e a segunda não duplica.
- [ ] **D-024 · Evento `connection` ainda sem payload real.** Nenhum chegou até 16/09/2026 (arquivo de webhooks local: 56 `messages`, 22 `messages_update`, 0 `connection`). Plano para provar sem mexer nos números reais: criar instância descartável pela API de admin, conectá-la no CRM pela tela, chamar `/instance/connect` (vai a `connecting` e dispara o evento), ler o corpo no arquivo de webhooks e apagar a instância. Bloqueado pela mesma queda do servidor da UAZAPI.
